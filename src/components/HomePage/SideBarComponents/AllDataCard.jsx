...
const AllDataCard = () => {
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      const response = await axiosPublic.delete(`/alltasks/${taskId}`); // Delete request
      if (!response.data) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }

      setCardData(cardData.filter(task => task._id !== taskId)); // Remove from state
        ...
        <Card key={data._id} className="shadow-2xl shadow-black min-w-64 sm:min-w-72 md:min-w-64 lg:min-w-72 xl:min-w-80 max-w-64 sm:max-w-72 md:max-w-64 lg:max-w-72 xl:max-w-80 mx-auto bg-green-100 rounded-md">
          ...
          <div className="w-full">
            <div className="flex gap-1 mx-auto justify-center">
              <Button><RiTodoFill /></Button>
              <Button className="bg-slate-500"><RiProgress1Fill /></Button>
              <Button className="bg-green-400"><MdDoneAll /></Button>
              <Button onClick={() => handleUpdateTask(data)} className="bg-stone-500"><BiEdit /></Button>
            </div>
          ...
          <Card className="bg-stone-300">
            <form onSubmit={handleUpdateSubmit}>
              ...
              <TextInput
                ...
                placeholder="Category"
                name="category"
                value={newTask.category || selectedTask?.category || ""}
                onChange={handleInputChange}
                required
              />
